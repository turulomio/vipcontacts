    ...
    <div>
        <v-data-table :headers="tableHeaders" :items="tableData" sort-by="dt_update" class="elevation-1" :key="refreshKey" >
              ...
              <template v-slot:item.type="{ item }">
                <span>{{ AddressTypeName(item.type) }}</span>
            </template>
        ...
        <v-card  class="login">
            ...
            <v-select :items="this.$store.state.catalogs.addresstype" v-model="selected.type" :label="$t('Select a type')"  item-text="display_name" item-value="value"  ></v-select>  
